...
(define (make-journalist name birthplace threshold)
  (let ((person (make-person name birthplace threshold)))
    (lambda (message)
      (cond ((eq? message 'talk)
	     (lambda (self)
	       (display-message (list "Teraz nie mogę, piszę artykuł. Przyjdź później."))))
            ((eq? message 'do-something)
	     (lambda (self matches student)
               (display-message (list "Masz ognia?"))
	       (if (eq? student (ask matches 'owner))
                   (display-message (list "Dzięki"))
                   (cond ((and (> threshold 0) (= (random threshold) 0))
                          (ask self 'act)
                          true)))))
            ((eq? message 'poke)
	     (lambda (self)
               (let ((possessions (ask self 'possessions)))
                 (if (null? possessions)
                     (display-message (list "Czemu mnie bijesz?"))
                     (ask self 'lose (car possessions))))))
	    (else (get-method person message))))))

(define (make&install-journalist name birthplace threshold)
  (let ((journalist  (make-journalist name birthplace threshold)))
    (ask journalist 'install)
    journalist))

;;; Komputer

(define (make-computer name birthplace)
  (let ((state 'off))
    (let ((person (make-thing name birthplace)))
      (lambda (message)
        (cond ((eq? message 'turned?)
               (lambda (self)
                 (lambda (self) state)))
              ((eq? message 'turn-on)
               (lambda (self)
                 (if (eq? state 'on)
                     (display-message (list "Już włączony"))
                     (set! state 'on))))
              ((eq? message 'turn-off)
               (lambda (self)
                 (if (eq? state 'off)
                     (display-message (list "Już wyłączony"))
                     (set! state 'off))))
              (else (get-method person message)))))))

(define (make&install-computer name birthplace)
  (let ((computer  (make-computer name birthplace)))
    (ask computer 'install)
    computer))
...
(define pracownia        (make-place 'pracownia))
(define IM               (make-place 'Instytut_Matematyczny))
...
(can-go-both-ways IM 'łącznik 'łącznik hol)
(can-go-both-ways piętro-wschód 'wschód 'zachód pracownia)
...
(define journalist1 (make&install-journalist 'dziennikarz hol 2))
(define journalist2 (make&install-journalist 'jbi wielka-zachodnia 3))
...
(define computer     (make&install-computer 'komputer pracownia))
(define matches      (make&install-thing 'matches IM))
...
(define (talk-to-journalist)
  (if (eq? hol (ask *player-character* 'place))
      (ask journalist1 'talk)
      (if (eq? wielka-zachodnia (ask *player-character* 'place))
          (ask journalist2 'talk)
          (display-message (list "Nie ma tu dziennkiarzy! Pójdę ich poszukać!")))))

(define (turn-on-computer)
  (ask computer 'turn-on)
  (display-message (list "*włącza się*")))

(define (turn-off-computer)
  (ask computer 'turn-off)
  (display-message (list "*wyłącza się*")))

(define (pick-up thing)
  (ask *player-character* thing 'take))

(define (help-journalist)
  (if (eq? hol (ask *player-character* 'place))
      (ask journalist1 'do-something matches *player-character*)
      (if (eq? wielka-zachodnia (ask *player-character* 'place))
          (ask journalist2 'do-something matches *player-character*)
          (display-message (list "Nie ma tu dziennkiarzy! Pójdę ich poszukać!")))))
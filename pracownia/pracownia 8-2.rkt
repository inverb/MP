...
(define (lazy-add-to-env x v env)
  (cons (list x v env) env))
...
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (caar env)) (if (and (list? (car env))
                                     (= (length (car env)) 3))
                                (eval-env (expr-env (car env))
                                          (env-env (car env)))
                                (cadar env))]
...
(define (env-for-lazy-let def env)
  (lazy-add-to-env
    ...
    (lazy-let-def-expr def)
    env))


(define (tests)
  (and
   (= 6 (eval '(lazy-let (x 5) (+ x 1))))
   (false? (val->bool (eval '(let (x 2)
                               (lazy-let (x (+ x 1))
                                         (if (= x 2) true false))))))
   (= 2 (eval '(lazy-let (x 1) (lazy-let (y (+ x 2)) (lazy-let (z (+ x y)) (- (+ z x) y))))))
   (= 3 (eval '(lazy-let (x 1) (lazy-let (x 2) (lazy-let (x 3) x)))))
   (= 5 (eval '(lazy-let (x 1) (let (x 2) (let (x 3) (let (x 4) (let (x 5) x)))))))
   (= 5 (eval '(let (x 1) (let (x 2) (let (x 3) (let (x 4) (lazy-let (x 5) x)))))))
   (= 1 (eval '(car (cons (lazy-let (x 1) (let (y 2) ((lambda (a) (/ a 1)) x))) (+ 1 1)))))
   (not (false? (eval '(pair? (let (x 1) (lazy-let (x (cons 1 2)) x))))))
   (= 55(eval '((lambda-rec (sum n) (lazy-let (s (+ n (sum (- n 1)))) (if (= n 1) 1 s))) 10)))
   (eq? 'kopytko (eval '(let (x 'slonko) (lazy-let (y 'kopytko) (lazy-let (x 2) (cond [(= x 1) x]
                                                                                      [(= x 2) (car (cdr y))]))))))
   ;;testy z treści zadania
   (= 3628800 (eval '((lambda-rec (fact n)
                          (lazy-let (t 1)
                                    (lazy-let (f (* n (fact (- n 1))))
                                              (if (= n 0) t f)))) 10)))
   (= 7 (eval '(lazy-let (x (/ 5 0)) 7)))
   (= 5 (eval '(let (x 4) (lazy-let (y (+ x 1)) (let (x 10) y)))))))

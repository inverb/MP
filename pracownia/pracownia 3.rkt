...
(define (solve-ax+by=1 a b)
  (define (iter c d x1 x2 y1 y2)
    (if(= d 0)
       (cons x2 y2)
       (let ((r (modulo c d))
             (q (floor (/ c d))))
         (iter d r (- x2 (* q x1)) x1 (- y2 (* q y1)) y1))))
    (iter a b 0 1 1 0))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ((encrypted-message (RSA-encrypt message recipient-public-key))
        (compressed-message (compress (string->intlist message))))
    (make-signed-message encrypted-message (RSA-convert-list (cons compressed-message null)  sender-private-key))))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ((message (RSA-decrypt (signed-message-message cyphertext) recipient-private-key))
        (signing (RSA-unconvert-list (signed-message-signature cyphertext) sender-public-key)))
    (let ((compressed-message (compress (string->intlist message))))
      (if(= compressed-message (car signing))
         message
         (error "Wrong signing")))))
